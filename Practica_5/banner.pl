...
cellsFilledForStartAndRotation(P, Xi,Yi,X,Y,1):-
    pieceSize(P,WP,HP),
    X2 is Xi+WP-1,
    Y2 is Yi+HP-1,
    between(Xi,X2,X),
    between(Yi,Y2,Y).
cellsFilledForStartAndRotation(P, Xi,Yi,X,Y,2):-
    pieceSize(P,WP,HP),
    X2 is Xi+HP-1,
    Y2 is Yi+WP-1,
    between(Xi,X2,X),
...
writeClauses(K):-
	eachPieceStartsAtMostOnce,
	onePieceInCellXY,
	noPieceInDots,
	defineUsedPieces,
	fillingWithRotation,
	fillingWithoutRotation,
  bePaintedImpliesStartSomewhere,
	doesntFillingWithRotation,
	doesntFillingWithoutRotation,
	noStartFromOutOfBoundsRotatedPieceX,
	noStartFromOutOfBoundsNoRotatedPieceX,
  noStartFromOutOfBoundsRotatedPieceY,
  noStartFromOutOfBoundsNoRotatedPieceY,
	atmostK(K),
	true,!.
...
doesntFillingWithRotation:-
  ...
  findall([X,Y], cellsFilledForStartAndRotation(P,Xi,Yi,X,Y,2), L),
  cell(X,Y), not(member([X,Y],L)),
  writeClause([\+rotated-P, \+pieceStarts-P-Xi-Yi, \+pieceCell-P-X-Y]),
  fail.
doesntFillingWithRotation.
...
doesntFillingWithoutRotation:-
  piece(P),
  contentsCellBanner(Xi, Yi, 'x'),
  findall([X,Y], cellsFilledForStartAndRotation(P,Xi,Yi,X,Y,1), L),
  cell(X,Y), not(member([X,Y],L)),
  writeClause([rotated-P, \+pieceStarts-P-Xi-Yi, \+pieceCell-P-X-Y]),
  fail.
doesntFillingWithoutRotation.
...
bePaintedImpliesStartSomewhere:-
  piece(P),
  findall(pieceStarts-P-Xi-Yi, contentsCellBanner(Xi,Yi,'x'), Lits),
  cell(X,Y),
  writeClause([\+pieceCell-P-X-Y | Lits]),
  fail.
bePaintedImpliesStartSomewhere.
...
piecesUsed(M,K):-
    findall(pieceStarts-P-X-Y, member(pieceStarts-P-X-Y,M), Pieces),
    length(Pieces, K),
    true.
